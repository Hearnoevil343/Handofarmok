...
Work happens on `feature/` or `fix/` branches off `develop` (see CLAUDE.md);
...
- [x] **Export fix: freezing temperatures were corrupted** — the exporter built
      ...
      `Int16Array`. Verified: all six painted layers in a 16-map export now
      match the preset files exactly.
...
## To build
- [x] **Sub-tile plate motion** (bilinear) — accepted, on `develop`:
...
- [x] **Desert stripes on all 8 procedural presets** — fixed on branch
      `fix/desert-stripes`, now on `develop`: rainfall belt now wanders and follows
...
- **0.2.1** — playtest bug fixes, time-scale changes, release
